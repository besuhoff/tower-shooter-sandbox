...
export class PixelMatrix {
  ...
  private _getChunkForPoint(point: Point): Point {
    ...
    return {
      x: exactPoint.x - (exactPoint.x % PixelMatrix.CHUNK_SIZE),
      y: exactPoint.y - (exactPoint.y % PixelMatrix.CHUNK_SIZE),
    };
  }

  private _getAvailableDropDeltaYForPoints(
    points: Point[],
    expectedDropDeltaY: number,
    ...
    if (points.length > 1) {
      const lowestPointsByXMap = points.reduce(
        (accum, point) => {
      ...
      bottomLinePoints = Object.entries(lowestPointsByXMap).map(([x, y]) => ({
        x: Number(x),
        y,
      }));
    ...
    const clampedDeltaY =
      Math.min(this.height - 1, lowestY + expectedDropDeltaY) - lowestY;
    let shiftY = 1;

    for (; shiftY <= clampedDeltaY; shiftY++) {
      // Break if non-air material is found below any point of the bottom line
      if (
        bottomLinePoints.some(
          (point) =>
            this._getCell({ y: point.y + shiftY, x: point.x }).type !== "air",
        )
      ) {
        break;
      }
  ...
  private _renderRegion(
    from: Point = { x: 0, y: 0 },
    to: Point = { x: this.width, y: this.height },
  ) {
    let exactFrom = this._normalizeCoordinates(from);
    let exactTo = this._normalizeCoordinates({ x: clamp(to.x, 0, this.width) - 1, y: clamp(to.y, 0, this.height) - 1 });
    this._renderer.clearRect(from, {
      width: to.x - from.x,
      height: to.y - from.y,
    });
  ...
  private _updatePoint(point: Point, material: Material) {
    ...
    this._matrixUpdatedChunks[`${chunkCoords.x}.${chunkCoords.y}`] =
      chunkCoords;
  }

  render() {
    Object.values(this._matrixUpdatedChunks).forEach((point) => {
      this._renderRegion(point, {
        x: point.x + PixelMatrix.CHUNK_SIZE,
        y: point.y + PixelMatrix.CHUNK_SIZE,
      });
    });
  ...
  updateSimulation() {
    ...
    if (this._lastUpdateTimestamp) {
      ...
      for (let i = (this.height - 1) * this.width - 1; i >= 0; i--) {
        ...
        if (
          cell.static ||
          this._materialMatrix[i + this.width].type !== "air" ||
          bannedPoints[`${i}`]
        ) {
          continue;
        }
        ...
        if (cell.droppingObjectId) {
          ...
          points = droppingObject.sprite
            .getCollisionPointsArray()
            .filter((point) => this._withinBoundaries(point));
        }

        const expectedDeltaY = Math.floor(speed * timeDelta);
        const actualDeltaY = this._getAvailableDropDeltaYForPoints(
          points,
          expectedDeltaY,
          cell,
        );
